...
public class Compilation {
    ...
    private Java createJavaTask(File runnerFile, File finalFile) {
        ...
        determineCompilerConstants(task);
    ...
    private void determineCompilerConstants(Java java) {
        ArrayList<CompilerConstant> constants = configuration.getCompilerConstants().getConstants();

        if (!constants.isEmpty()) {
            for (CompilerConstant constant : constants) {
                Argument compilerConstantPath = java.createArg();
                compilerConstantPath.setValue("-define+=" + constant.getValue());
            }
        }
    }
}
